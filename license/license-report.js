...
if (nonAllowed.length > 0) {
  ...
  for (const [license, packages] of Object.entries(byLicense).sort()) {
    ...
    if (packages.length - rootPackages.length > 0) {
      console.log(
        `   📦 Transitive dependencies (${packages.length - rootPackages.length})`,
      );
      if (packages.length <= 10) {
        packages
          .filter((p) => !p.isRootDep)
          .forEach((p) => console.log(`      - ${p.baseName}`));
      } else {
        packages
          .filter((p) => !p.isRootDep)
          .slice(0, 5)
          .forEach((p) => console.log(`      - ${p.baseName}`));
        console.log(
          `      ... and ${packages.length - rootPackages.length - 5} more`,
        );
      }
    }
  }
...
const report = {
  summary: {
    ...
    rootDependenciesWithNonAllowed: nonAllowed.filter((p) => p.isRootDep)
      .length,
  },
